...
def main() -> int:
    ...
    for notebook_rel in enabled:
        ...
        try:
            ...
            # No bootstrap cell to exercise; check_colab_notebooks --strict is
            # the gate for "must have one", so treat it as nothing to smoke test.
            print(f"  SKIP: {exc}")
            skipped.append(notebook_rel)
            continue
        try:
            execute_smoke_notebook(
                notebook_path, smoke_nb, source_indices, timeout=args.timeout
            )
        except Exception as exc:  # noqa: BLE001 - report any execution failure
            print(f"  FAIL: {exc}")
            failed.append(notebook_rel)
            continue
        passed.append(notebook_rel)

    print(
        f"\nColab smoke tests: {len(passed)} passed, "
        f"{len(skipped)} skipped (no bootstrap cell), {len(failed)} failed."
    )
    if failed:
