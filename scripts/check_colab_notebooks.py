...
def run_check(manifest_path: Path, strict: bool = False) -> int:
    ...
    # A manifest entry whose file is gone is already reported by
    # validate_manifest ("references notebooks that do not exist"); skip it here
    # so a stale entry gives that message rather than a FileNotFoundError.
    present_enabled = [n for n in sorted(enabled) if (REPO_ROOT / n).is_file()]
    present_disabled = [n for n in sorted(disabled) if (REPO_ROOT / n).is_file()]

    for notebook_path in present_enabled:
        errors.extend(
            validate_enabled_notebook(REPO_ROOT / notebook_path, repo, git_ref)
        )

    for notebook_path in present_disabled:
